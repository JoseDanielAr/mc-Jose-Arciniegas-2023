...
while switch==0:
    ...
    if R==1:
        G3=G1.union(G2)
        print(G1.union(G2), "cardinalidad: ", len(G3))
        R=int(input("¿Realizar otra operación?\n1: Sí\nOtro valor: No\n"))
        if R!=1:
         switch=1
    elif R==2:
        G3=G1.intersection(G2)
        print(G1.intersection(G2), "cardinalidad: ", len(G3))
        R=int(input("¿Realizar otra operación?\n1: Sí\nOtro valor: No\n"))
        if R!=1:
         switch=1
    elif R==3:
        G3=G1.difference(G2)
        print(G1.difference(G2), "cardinalidad: ", len(G3))
        R=int(input("¿Realizar otra operación?\n1: Sí\nOtro valor: No\n"))
        if R!=1:
         switch=1
    elif R==4:
        G3=G1.symmetric_difference(G2)
        print(G1.symmetric_difference(G2), "cardinalidad: ", len(G3))
        R=int(input("¿Realizar otra operación?\n1: Sí\nOtro valor: No\n"))
        if R!=1:
         switch=1
    else:
